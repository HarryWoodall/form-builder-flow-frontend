<script lang="ts">
  ...
  const getAvailibilityClass = (IsAvailable: boolean | undefined) => {
    return `pl-2 ${IsAvailable ? "font-bold text-orange-100" : "text-orange-300"}`;
  };
</script>

<div>
  {#if isVisible}
    <Card class="details-card bg-orange-400" style="min-width: 400px">
      ...
      <P class="text-orange-200 mb-4" size="base">{form.BaseURL}</P>

      <P class="font-bold text-white" size="lg">Start page url</P>
      <P class="text-orange-200 mb-4" size="base">{form.StartPageUrl}</P>
  ...
  {:else}
    <Button on:click={() => (isVisible = true)}>Details</Button>
  {/if}
</div>
